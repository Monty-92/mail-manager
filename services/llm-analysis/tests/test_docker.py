"""Docker container tests for the LLM analysis service.
...
SERVICE_NAME = "llm-analysis"
IMAGE_TAG = f"mailmanager-llm-analysis:test"
CONTAINER_NAME = "test-llm-analysis"
CONTAINER_PORT = 8003
HOST_PORT = 18003
...
def docker_image():
    """Build the Docker image for the LLM analysis service."""
...
class TestDockerContainer:
    ...
    def test_analyze_endpoint_registered(self, docker_container):
        """POST /analyze/{email_id} route is accessible (will 500 without DB, but route exists)."""
        resp = httpx.post(f"{docker_container}/analyze/test-id", timeout=5.0)
        # Without a real DB, expect 500 — but route is registered (not 404 for the path)
        assert resp.status_code in (200, 500)

    def test_batch_endpoint_registered(self, docker_container):
        """POST /analyze/batch route is accessible."""
        resp = httpx.post(f"{docker_container}/analyze/batch?limit=1", timeout=5.0)
        assert resp.status_code in (200, 500)

    def test_get_analysis_endpoint_registered(self, docker_container):
        """GET /analyze/{email_id} route is accessible."""
        resp = httpx.get(f"{docker_container}/analyze/test-id", timeout=5.0)
    ...
    def test_openapi_docs_available(self, docker_container):
        ...
        assert resp.status_code == 200
        assert resp.json()["info"]["title"] == "LLM Analysis Service"
