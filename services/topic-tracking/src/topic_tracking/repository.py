...
async def get_email_for_topics(email_id: str) -> EmailForTopics | None:
    ...
    row = await pool.fetchrow(
        """
        SELECT e.id, e.subject, translate(e.embedding::text, '[]', '{}')::float[] AS embedding,
...
async def find_topic_by_name(name: str) -> Topic | None:
    ...
    pool = await get_pool()
    row = await pool.fetchrow(
        "SELECT id, name, translate(embedding::text, '[]', '{}')::float[] AS embedding, snapshots, created_at, updated_at FROM topics WHERE LOWER(name) = LOWER($1)",
        name,
    )
    if row is None:
        return None
...
async def find_similar_topics(embedding: list[float], threshold: float = 0.3, limit: int = 5) -> list[tuple[Topic, float]]:
    ...
    rows = await pool.fetch(
        """
        SELECT id, name, translate(embedding::text, '[]', '{}')::float[] AS embedding, snapshots, created_at, updated_at,
...
async def create_topic(name: str, embedding: list[float] | None = None) -> Topic:
    ...
    row = await pool.fetchrow(
        ...
        RETURNING id, name, translate(embedding::text, '[]', '{}')::float[] AS embedding, snapshots, created_at, updated_at
        """,
        name,
        embedding_str,
    )
...
async def get_topic_by_id(topic_id: str) -> Topic | None:
    ...
    row = await pool.fetchrow(
        """
        SELECT t.id, t.name, translate(t.embedding::text, '[]', '{}')::float[] AS embedding, t.snapshots, t.created_at, t.updated_at,
